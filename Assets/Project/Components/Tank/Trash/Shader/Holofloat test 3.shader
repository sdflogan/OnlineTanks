...
Shader "FX/Hologram Effect" {
    Properties{
        ...
        _BigWorldScale("Big Line Ammout", Range (1,200)) = 20
        _Speed ("Big line speed", range (1,100)) = 70
    }
...
#include "UnityCG.cginc"
    ...
    fixed4 frag(v2f i) : SV_Target
    {
        ...
        // big scanline up
        float bigfracline = frac((i.wpos.y * _BigWorldScale) + _Time.x * _Speed) - frac((i.wpos.y * _WorldScale) + _Time.y);// big gradient line
        float scanfracline = step(bigfracline,.5);

        fixed4 col = text + (bigfracline * 0.4 * _TintColor) ;// end result color

        col.a = 0.3 * ( scanfracline + bigfracline);// alpha based on scanlines and rim
